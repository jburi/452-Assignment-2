 ...
 * Assignment 2
 * Manages player's weapon (Added elemental damage and behavior)
 */
...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // weapon switch handling + Element
        if (!isAiming &&
            ...
            (m_WeaponSwitchState == WeaponSwitchState.Up || m_WeaponSwitchState == WeaponSwitchState.Down))
        {
            ...
            else
            {
                ...
                if (switchWeaponInput != 0)
                {
                    ...
                    {
                        activeWeapon.element = new Fire();
                        activeWeapon.element.Element();

                    }
                    else if (switchWeaponInput == 2)
                    {
                        activeWeapon.element = new Earth();
                        activeWeapon.element.Element();
                    }
                    else if (switchWeaponInput == 3)
                    {
                        activeWeapon.element = new Water();
                        activeWeapon.element.Element();
                    }
		...
		//Extra Element Damage for certain enemies
        if (isPointingAtEnemy == true)
		{
			Physics.Raycast(weaponCamera.transform.position, weaponCamera.transform.forward, out RaycastHit hit, 1000, -1, QueryTriggerInteraction.Ignore);
			WeaponController enemyWeapon = hit.collider.GetComponent<WeaponController>();

            ElementalDamage(activeWeapon, enemyWeapon);
		}
    ...
    //Each element damages each other
    public void ElementalDamage(WeaponController playerWeapon, WeaponController enemyWeapon)
	{
        if (playerWeapon.element is Fire)
        {
            if (enemyWeapon.element is Earth)
            {
                Damageable player = playerWeapon.GetComponentInParent<Damageable>();
                player.damageMultiplier = 1.5f;
            }
            else if (enemyWeapon.element is Water)
            {
                Damageable player = playerWeapon.GetComponentInParent<Damageable>();
                player.damageMultiplier = 0.5f;
            }
        }
        else if (playerWeapon.element is Earth)
        {
            if (enemyWeapon.element is Water)
            { 
                Damageable player = playerWeapon.GetComponentInParent<Damageable>();
                player.damageMultiplier = 1.5f;
            }
            else if (enemyWeapon.element is Fire)
            {
                Damageable player = playerWeapon.GetComponentInParent<Damageable>();
                player.damageMultiplier = 0.5f;
            }
        }
        else if (playerWeapon.element is Water)
		{
            if (enemyWeapon.element is Fire)
            {
                Damageable player = playerWeapon.GetComponentInParent<Damageable>();
                player.damageMultiplier = 1.5f;
            }
    ...
    //Get element type 
    /*
    public int GetElement(WeaponController active)
    {
